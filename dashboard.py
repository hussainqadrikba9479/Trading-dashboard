...
def run_bot():
    ...
    # Load Data
    trading_mode = "Swing Trading (D1 + H4)"
    ...
    if not strong.empty and not weak.empty:
        for _, s in strong.iterrows():
            for _, w in weak.iterrows():
                ...
                # NAYI LOGIC: Sirf Technicals (Price Action + Volume) ko as a filter use karo
                if ("✅" in s['Volume Confirm'] or "✅" in w['Volume Confirm']):
                    
                    # COT aur OI ka data uthao AI ko dene ke liye
                    c1_cot, c1_oi = "Neutral", "Neutral"
                    if not cot_df.empty:
                        s_sent = cot_df[cot_df['Instrument'].str.contains(c1, case=False)]['Direction'].values
                        if len(s_sent) > 0: c1_cot = s_sent[0]
                    if not oi_df.empty and 'Status' in oi_df.columns:
                        s_oi_status = oi_df[oi_df['Instrument'] == c1]['Status'].values
                        if len(s_oi_status) > 0: c1_oi = s_oi_status[0]
                    ...
                    try:
                        ...
                        print(f"🔥 Technical Setup Detected: {action} {pair}")
                        
                        # AI Verification & Scoring (1 to 10)
                        verdict = "⚠️ AI verification offline."
                        if ai_model:
                            print("🤖 AI Scoring the Setup...")
                            news_str = "\n".join([n['title'] for n in live_news]) if live_news else "No major news"
                            
                            prompt = f"""
                            Expert Quant Analyst:
                            Setup: {action} {pair}
                            Technicals: Strong {c1} (Score: {s['Score']}) vs Weak {c2} (Score: {w['Score']}). Volume is Confirmed.
                            Institutional Data: {c1} COT Direction is '{c1_cot}'. {c1} Daily OI is '{c1_oi}'.
                            Live News: {news_str}
                            
                            Task: Evaluate how well the Institutional Data & News align with the Technical Setup.
                            1. Give an alignment score out of 10 (e.g., Score: 8/10).
                            2. Give a 2-line reasoning in Roman Urdu.
                            """
    ...
    if not found_setup:
        print("💤 No technical setups found with volume confirmation right now. Sleeping till next cycle.")
